...
const adminPageRedirect=await handleAnalytics(req('/admin/ko'),env);assert.equal(adminPageRedirect.status,302);assert.match(adminPageRedirect.headers.get('location'),/\/admin\/login\/ko$/);
assert.equal((await handleAnalytics(req('/admin/login/ko'),env)).status,200);
...
assert.equal((await handleAnalytics(req('/admin/ko','GET',null,{Cookie:cookie}),env)).status,200);
