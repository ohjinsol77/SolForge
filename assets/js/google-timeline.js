(function () {
  ...
  const copy = lang === "en" ? {
    ...
    noMap: "The map library could not be loaded. Statistics are still available.", playback: "Playing", stopped: "Ready", notAvailable: "—", day: "days", hours: "hours", visits: "visits", km: "km", min: "min", entries: "records"
  } : {
  ...
  function parseLatLng(value) {
    if (typeof value === "string") { const match = value.match(/(-?\d+(?:\.\d+)?)\s*°?\s*,\s*(-?\d+(?:\.\d+)?)\s*°?/); if (match) return { lat: Number(match[1]), lng: Number(match[2]) }; }
  ...
  function pointFromE7(location) { if (!location || !Number.isFinite(location.latitudeE7) || !Number.isFinite(location.longitudeE7)) return null; return { lat: location.latitudeE7 / 1e7, lng: location.longitudeE7 / 1e7, accuracy: number(location.accuracy) }; }
  ...
  function normalizeLocations(data, warnings) {
    const points = (data.locations || data).map((location) => {
      const point = pointFromE7(location); if (!point) return null; point.time = isoFromOld(location.timestampMs || location.timestamp); point.accuracy = number(location.accuracy); return point;
  ...
  function inspectData(data) {
    ...
    if (Array.isArray(data)) { if (data.some((item) => item && (item.timestampMs || item.latitudeE7))) { format = "locations array"; records = normalizeLocations(data, warnings); } else errors.push(unsupportedMessage()); }
  ...
  function applyFilters() { state.filtered = filterRecords(); renderStats(); renderInsights(); renderMap(); }
  ...
  function playRoute() { if (!state.map || state.playbackPoints.length < 2) return; if (state.playing) { stopPlayback(); return; } state.playing = true; els.play.textContent = lang === "en" ? "Pause" : "일시정지"; if (!state.playbackMarker) state.playbackMarker = window.L.marker([state.playbackPoints[0].lat, state.playbackPoints[0].lng], { icon: window.L.divIcon({ className: "timeline-route-marker", iconSize: [16, 16], iconAnchor: [8, 8] }) }).addTo(state.map); const started = performance.now(); const duration = Math.max(3500, Math.min(18000, state.playbackPoints.length * 100)); const frame = (now) => { if (!state.playing) return; const progress = Math.min(1, (now - started) / duration); const index = Math.min(state.playbackPoints.length - 1, Math.floor(progress * state.playbackPoints.length)); const point = state.playbackPoints[index]; state.playbackMarker.setLatLng([point.lat, point.lng]); els.progress.value = Math.round(progress * 1000); els.playbackLabel.textContent = `${copy.playback} · ${Math.round(progress * 100)}%`; if (progress < 1) state.playbackTimer = requestAnimationFrame(frame); else { stopPlayback(); els.playbackLabel.textContent = copy.stopped; } }; state.playbackTimer = requestAnimationFrame(frame); }

  els.choose.addEventListener("click", () => els.input.click()); els.input.addEventListener("change", () => handleFile(els.input.files?.[0])); els.reset.addEventListener("click", () => { els.input.value = ""; els.meta.hidden = true; setValidation("", lang === "en" ? "Choose a Timeline JSON file to begin." : "Timeline JSON 파일을 선택하면 시작합니다."); resetDashboard(); }); [els.from, els.to, els.type, els.mode, els.minDuration].forEach((element) => element.addEventListener("input", applyFilters)); els.filterReset.addEventListener("click", () => { configureFilters(); els.type.value = "all"; els.mode.value = "all"; els.minDuration.value = "0"; applyFilters(); }); els.play.addEventListener("click", playRoute); els.fit.addEventListener("click", () => { if (state.map && state.playbackPoints.length) state.map.fitBounds(state.playbackPoints.map((point) => [point.lat, point.lng]), { padding: [24, 24], maxZoom: 15 }); });
