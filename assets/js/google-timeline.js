(function () {
  ...
  els.choose.addEventListener("click", () => els.input.click()); els.input.addEventListener("change", () => handleFile(els.input.files?.[0])); els.reset.addEventListener("click", () => { els.input.value = ""; els.meta.hidden = true; setValidation("", lang === "en" ? "Choose a Timeline JSON file to begin." : "Timeline JSON 파일을 선택하면 시작합니다."); resetDashboard(); }); [els.from, els.to, els.type, els.mode, els.minDuration].forEach((element) => element.addEventListener("input", () => { if (element === els.from || element === els.to) els.preset.value = "custom"; applyFilters(); })); els.preset.addEventListener("change", () => { applyDatePreset(els.preset.value); applyFilters(); }); els.basis.addEventListener("input", () => { if (els.preset.value === "custom" || els.preset.value === "all") els.preset.value = "custom"; else applyDatePreset(els.preset.value); applyFilters(); }); els.filterReset.addEventListener("click", () => { configureFilters(); els.type.value = "all"; els.mode.value = "all"; els.minDuration.value = "0"; applyFilters(); }); els.play.addEventListener("click", playRoute); els.progress.addEventListener("input", () => { stopPlayback(); setPlaybackPosition(els.progress.value); }); els.fit.addEventListener("click", () => { if (state.map && state.playbackPoints.length) state.map.fitBounds(state.playbackPoints.map((point) => [point.lat, point.lng]), { padding: [24, 24], maxZoom: 15 }); });
